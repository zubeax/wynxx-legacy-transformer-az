       ...
       PROCEDURE DIVISION USING TXN-TABLE LK-AGG-TABLE.
           ...
           PERFORM VARYING WS-I FROM 1 BY 1
                     UNTIL WS-I > TXN-COUNT

              MOVE 'N' TO FOUND-FLAG

              PERFORM VARYING WS-J FROM 1 BY 1
                      UNTIL WS-J > LK-AGG-NUM
                         OR FOUND-FLAG = 'Y'

                 SET LK-AG-IDX TO WS-J

                 IF     TX-SERVICE(WS-I) = LK-AE-SERVICE(WS-J)
                    AND TX-CHANNEL(WS-I) = LK-AE-CHANNEL(WS-J)
                    AND TX-SCA(WS-I)     = LK-AE-SCA(WS-J)
                 THEN
                    ADD 1                   TO LK-AE-COUNT(WS-J)
                    ADD TX-AMOUNT-EUR(WS-I) TO LK-AE-TOTAL(WS-J)
