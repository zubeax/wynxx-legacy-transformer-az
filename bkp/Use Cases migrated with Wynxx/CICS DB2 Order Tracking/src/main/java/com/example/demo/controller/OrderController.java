...
@Tag(name = "Order Management", description = "APIs for managing client orders")
...
public class OrderController {
    ...
    @Operation(summary = "Create a new order", description = "Create a new order for a customer")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Update an existing order", description = "Update order details by ID (only for non-terminal orders)")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Order updated successfully"),
        @ApiResponse(responseCode = "400", description = "Invalid request data or order in terminal status"),
        @ApiResponse(responseCode = "404", description = "Order not found"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    @PutMapping("/{id}")
    ...
    @Operation(summary = "Delete an order", description = "Delete an order by ID (only terminal orders can be deleted)")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "204", description = "Order deleted successfully"),
        @ApiResponse(responseCode = "400", description = "Order is not in a terminal status"),
        @ApiResponse(responseCode = "404", description = "Order not found"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    @DeleteMapping("/{id}")
    ...
    @Operation(summary = "Confirm an order", description = "Confirm a pending order")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Order confirmed successfully"),
        @ApiResponse(responseCode = "400", description = "Order cannot be confirmed in current status"),
        @ApiResponse(responseCode = "404", description = "Order not found"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    ...
    public ResponseEntity<OrderResponseDto> confirmOrder(
            @Parameter(description = "Order ID", required = true) @PathVariable Long id,
            @Parameter(description = "Name or ID of the person confirming") @RequestParam(required = false) String confirmedBy) {
        OrderResponseDto response = orderService.confirmOrder(id, confirmedBy);
        return ResponseEntity.ok(response);
    }

    @Operation(summary = "Process an order", description = "Start processing a confirmed order")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Order processing started successfully"),
        @ApiResponse(responseCode = "400", description = "Order must be CONFIRMED before processing"),
        @ApiResponse(responseCode = "404", description = "Order not found"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    ...
    public ResponseEntity<OrderResponseDto> processOrder(
            @Parameter(description = "Order ID", required = true) @PathVariable Long id,
            @Parameter(description = "Name or ID of the person processing") @RequestParam(required = false) String processedBy) {
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "400", description = "Order cannot be shipped in current status"),
        @ApiResponse(responseCode = "404", description = "Order not found"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    ...
    public ResponseEntity<OrderResponseDto> shipOrder(
            @Parameter(description = "Order ID", required = true) @PathVariable Long id,
            @RequestBody ShipOrderRequestDto request) {
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "400", description = "Order must be SHIPPED before marking as delivered"),
        @ApiResponse(responseCode = "404", description = "Order not found"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    ...
    public ResponseEntity<OrderResponseDto> deliverOrder(
            @Parameter(description = "Order ID", required = true) @PathVariable Long id,
            @Parameter(description = "Name or ID of the person confirming delivery") @RequestParam(required = false) String deliveredBy) {
        OrderResponseDto response = orderService.deliverOrder(id, deliveredBy);
        return ResponseEntity.ok(response);
    }

    @Operation(summary = "Cancel an order", description = "Cancel an order that is not yet shipped or delivered")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Update order status", description = "Manually update the status of an order")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Record payment for an order", description = "Record a payment against an order")
    @ApiResponses(value = {
    ...
    public ResponseEntity<OrderResponseDto> recordPayment(
            ...
            @Parameter(description = "Payment reference/transaction ID") @RequestParam(required = false) String paymentReference) {
        OrderResponseDto response = orderService.recordPayment(id, amount, paymentReference);
        return ResponseEntity.ok(response);
    }
